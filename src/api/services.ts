...
export const planApi={ current:()=>api<{plan:T.Plan;limits:{maxVenues:number;maxUsers:number;maxAddons:number;maxPackages:number;maxGalleryImages:number};features:{advancedPricing:boolean;removeBranding:boolean;employeeRole:boolean;advancedDashboard:boolean;plusAnalytics:boolean}}>('/plan') };
...
export const usersApi={ list:()=>api<T.User[]>('/users'), me:()=>api<T.UserProfile>('/users/me'), updateMe:(data:Partial<T.UserProfile>)=>api<{profile:T.UserProfile;token:string}>('/users/me',{method:'PUT',body:JSON.stringify(data)}), changePassword:(data:{currentPassword:string;newPassword:string})=>api<void>('/users/me/password',{method:'PUT',body:JSON.stringify(data)}), setActive:(id:number,active:boolean)=>api<T.User>(`/users/${id}/active?active=${active}`,{method:'PATCH'}), remove:(id:number)=>api<void>(`/users/${id}`,{method:'DELETE'}), create:(data:{name:string;email:string;password:string;role:T.Role})=>api<T.User>('/users',{method:'POST',body:JSON.stringify(data)}) };
export const venuesApi={ list:()=>api<T.Venue[]>('/venues'), get:(id:number)=>api<T.Venue>(`/venues/${id}`), create:(data:Partial<T.Venue>)=>api<T.Venue>('/venues',{method:'POST',body:JSON.stringify(data)}), update:(id:number,data:Partial<T.Venue>)=>api<T.Venue>(`/venues/${id}`,{method:'PUT',body:JSON.stringify(data)}), remove:(id:number)=>api<void>(`/venues/${id}`,{method:'DELETE'}), setActive:(id:number,active:boolean)=>api<T.Venue>(`/venues/${id}/active?active=${active}`,{method:'PATCH'}) };
...
export const subscriptionApi={ me:()=>api<T.Subscription>('/subscription'), change:(plan:T.Plan,billingCycle:string)=>api<{id:string|null;url:string|null}>('/subscription/change',{method:'POST',body:JSON.stringify({plan,billingCycle})}), cancel:()=>api<void>('/billing/cancel',{method:'POST'}) };
...
export const dashboardApi={analytics:(period:'WEEK'|'MONTH')=>api<T.DashboardAnalytics>(`/dashboard/analytics?period=${period}`)};
export const passwordApi={forgot:(email:string)=>api<void>('/password/forgot',{method:'POST',body:JSON.stringify({email})}),reset:(token:string,password:string)=>api<void>('/password/reset',{method:'POST',body:JSON.stringify({token,password})}),platformForgot:(email:string)=>api<void>('/platform/password/forgot',{method:'POST',body:JSON.stringify({email})}),platformReset:(token:string,password:string)=>api<void>('/platform/password/reset',{method:'POST',body:JSON.stringify({token,password})})};
export const platformApi={ metrics:()=>api<T.PlatformMetrics>('/platform/admin/metrics',{},true), tenants:()=>api<T.Tenant[]>('/platform/admin/tenants',{},true), suspension:(id:number,suspended:boolean)=>api<T.Tenant>(`/platform/admin/tenants/${id}/suspension?suspended=${suspended}`,{method:'PATCH'},true), updateTenant:(id:number,data:Partial<T.Tenant>)=>api<T.Tenant>(`/platform/admin/tenants/${id}`,{method:'PUT',body:JSON.stringify(data)},true), owner:(id:number,data:{name?:string;email?:string;username?:string})=>api<T.User>(`/platform/admin/tenants/${id}/owner`,{method:'PUT',body:JSON.stringify(data)},true), usage:(days=30)=>api<T.PlatformUsagePoint[]>(`/platform/admin/usage?days=${days}`,{},true), alerts:()=>api<T.PlatformAlert[]>('/platform/admin/alerts',{},true), subscription:(id:number,data:T.Subscription)=>api<T.Subscription>(`/platform/admin/tenants/${id}/subscription`,{method:'PUT',body:JSON.stringify(data)},true) };